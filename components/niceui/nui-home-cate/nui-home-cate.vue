<template>
	...
	<view class="container">
		...
		<!-- #ifdef MP-WEIXIN -->
		<view class="forum_ad">
			<ad-custom class=" forum_ad_img" unit-id="adunit-f4863764a2e44ac0"></ad-custom>
		</view>
		<!-- #endif -->
		...
		<view class="forum_cate">
			...
			<view v-for="(item,index) in catelist" :key="index" class="cate_list">
				...
				<view v-for="(item1,index1) in item.children" :key="index1" class="cate_info" v-if="item.children.length > 0">
					<view class="cate_item" @click="itemClick(item1)">
						...
						<view class="cate_num">帖数: {{item1.threadCount}}</view>

					</view>
				</view>
				<view v-if="item.children.length<1 "class="cate_info">
					<view class="cate_item" @click="itemClick(item)">
						<view class="cate_icon">
							<image class="img"
								:src="'https://ui-avatars.com/api/?background=random&length=1&font-size=0.23&name='+item.name">
						</view>
						<view class="cate_name">{{item.name}}</view>
						<view class="cate_num">帖数: {{item.threadCount}}</view>
	...
	export default {
		...
		methods: {
			...
			itemClick(data) {
				// 跳转到内部窗口并且传递链接
				//var url = `/pages/webview/webview?url=` + API_URL + 'thread/' + data.threadId;
				var url = `/pages/list/list?id=` + data.pid + '&name=' + data.name;
				uni.navigateTo({
					url: url
				});
			},
